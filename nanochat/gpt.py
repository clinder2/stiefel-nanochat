...
sys.path.append("/Users/christopherlinder/Desktop/OptimizerTesting/ShampooExperiment/StackedShampoo")
sys.path.append("/storage/home/hcoda1/7/clinder9/r-mtao8-0/OptimizerTesting/ShampooExperiment")
from StiefelOptimizers import StiefelSGD, StiefelAdam
import StackedShampoo
...
class GPT(nn.Module):
    ...
    def setup_optimizer_muon(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
                        weight_decay=0.0, adam_betas=(0.8, 0.95), scalar_lr=0.5):
        print("init just AdamWMuon optimizer")
    ...
    def setup_optimizer_stiefel(self, unembedding_lr=0.004, embedding_lr=0.6, matrix_lr=0.04,
                        ...
                        stiefel_betas=(0.8, 0.95), stiefel_momentum=.9, stiefel_type='Adam'):
        print(f"init AdamWMuon and Stiefel{stiefel_type} optimizers")
        model_dim = self.config.n_embd
        matrix_params = []
        value_embeds_params = list(self.value_embeds.parameters())
        embedding_params = list(self.transformer.wte.parameters())
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
                        weight_decay=0.0, adam_betas=(0.8, 0.95), scalar_lr=0.5, beta2=.95, chol=False):
        model_dim = self.config.n_embd
        matrix_params = list(self.transformer.h.parameters())
        value_embeds_params = list(self.value_embeds.parameters())
        embedding_params = list(self.transformer.wte.parameters())
        ...
        for shape in sorted({p.shape for p in matrix_params}):
            ...
            shampoo_param_groups.append(dict(
                kind='shampoo', params=group_params, lr=matrix_lr,
                momentum=0.95, ns_steps=5, beta2=beta2, weight_decay=weight_decay,
            ))
        optimizer = MuonAdamW(param_groups)
        s_optimizer = StackedShampoo(shampoo_param_groups, chol=chol)
        for group in optimizer.param_groups:
            group["initial_lr"] = group["lr"]
        for group in s_optimizer.param_groups:
            group["initial_lr"] = group["lr"]
        return optimizer, s_optimizer
    
    def setup_optimizer_shampoo(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
                        weight_decay=0.0, adam_betas=(0.8, 0.95), scalar_lr=0.5, shampoo_lr=0.02, 
                        shampoo_betas=(0.8, 0.95), shampoo_momentum=.9, cholesky=False):
        model_dim = self.config.n_embd
        matrix_params = list(self.transformer.h.parameters())
        value_embeds_params = list(self.value_embeds.parameters())
        embedding_params = list(self.transformer.wte.parameters())
        ...
        for shape in sorted({p.shape for p in matrix_params}):
            ...
            shampoo_param_groups.append(dict(
                ...
                momentum=shampoo_momentum, beta2=shampoo_betas[1], weight_decay=weight_decay,
            ))
