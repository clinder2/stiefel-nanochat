...
class GPT(nn.Module):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
                        ...
                        stiefel_betas=(0.8, 0.95), stiefel_momentum=.9, stiefel_type='SGD', other='None'):
        ...
        for h in self.transformer['h']:
            for n, p in h.named_parameters():
                if "c_q" not in n and "c_k" not in n:
                    ...
                    QK_params.append(p)
        assert len(list(self.parameters())) == (len(QK_params) + len(matrix_params) + len(embedding_params) +
        ...
        param_groups.append(dict(kind='adamw', params=matrix_params, lr=matrix_lr, betas=adam_betas, eps=1e-10, weight_decay=weight_decay))
        if other=='muon_standard':
        ...
        if other_optimizer is not None:
            for group in other_optimizer.param_groups:
                group["initial_lr"] = group["lr"]
        return optimizer, other_optimizer
